...
const AccommodationRow = ({accommodation, isSearch, searchRequest}: {accommodation: Accommodation, isSearch: boolean, searchRequest?: SearchRequest}) => {
    const data = accommodation.images.length > 0 ? (accommodation.images[0] as any).imageData : "";
        ...
        >
            ...
            <Box display="flex" flexDirection="row" justifyContent='space-between'>
                ...
                <Box display='flex' flexDirection='column'>
                    ...
                    {!isSearch && <Box marginTop={1}>
                            <Button variant="outlined" style={{width: '130px'}} onClick={() => setOpenRatingsDialog(true)}>
                                Ratings
                            </Button>
                        </Box>
                    }
            ...
            {openRatingsDialog && <RatingsDialog open={openRatingsDialog} setOpen={setOpenRatingsDialog} accommodationId={accommodation.id} accommodationName={accommodation.name}/>}
        </Box>
    );
};
